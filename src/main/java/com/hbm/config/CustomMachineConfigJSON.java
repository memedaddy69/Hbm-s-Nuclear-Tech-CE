...
public class CustomMachineConfigJSON {
	...
	public static void writeDefault(File config) {
		...
		try {
				...
				for(int y = -1; y <= 1; y++) {
					for(int z = 0; z <= 2; z++) {
						if(!(x == 0 && y == 0 && z == 1) && !(x == 0 && z == 0)) {
							...
							writer.name("block").value(y == 0 ? "hbm:cm_sheet" : "hbm:cm_block");
			...
			writer.beginObject().setIndent("");
			writer.name("block").value("hbm:cm_port");
			...
			for(int i = 0; i < machines.size(); i++) {
				...
				if(machineObject.has("recipeShape") && machineObject.has("recipeParts")) {
					try {
						...
						tag.setString("machineType", configuration.unlocalizedName);
				...
				for(int j = 0; j < components.size(); j++) {
					...
					compDef.block = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(compObject.get("block").getAsString()));
