...
public class GUIFusionKlystron extends GuiInfoContainer {
    ...
    private static final ResourceLocation texture = new ResourceLocation(Tags.MODID + ":textures/gui/reactors/gui_fusion_klystron.png");
    ...
    public void drawScreen(int mouseX, int mouseY, float interp) {
        ...
        drawCustomInfoStat(mouseX, mouseY, guiLeft + 43, guiTop + 71, 18, 18, mouseX, mouseY, new String[]{TextFormatting.RED + "<- " + TextFormatting.RESET + BobMathUtil.getShortNumber(klystron.output) + "KyU / " + BobMathUtil.getShortNumber(klystron.outputTarget) + "KyU"});
        klystron.compair.renderTankInfo(this, mouseX, mouseY, guiLeft + 76, guiTop + 71, 18, 18);
        drawCustomInfoStat(mouseX, mouseY, guiLeft + 115, guiTop + 71, 18, 18, mouseX, mouseY, new String[]{TextFormatting.GREEN + "-> " + TextFormatting.RESET + BobMathUtil.getShortNumber(klystron.output) + "HE / " + BobMathUtil.getShortNumber(klystron.outputTarget) + "HE"});

    }

    @Override
    protected void mouseClicked(int i, int j, int button) throws IOException {
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        String name = this.klystron.hasCustomName() ? this.klystron.getName() : I18n.format(this.klystron.getDefaultName());
